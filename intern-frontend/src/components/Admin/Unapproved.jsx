...
const Unapproved = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get("http://localhost:5000/admin/getUnapprovedData").then((res) => {
      console.log(res);
  ...
  const approve = (id) => {
    ...
    axios
      .put("http://localhost:5000/admin/allowSuppliers", {
        _id: id,
      ...
      .then(async (res) => {
        console.log(res);
        await axios
          .get("http://localhost:5000/admin/getUnapprovedData")
          .then((res) => {
            console.log(res);
            setUnapproved(res.data);
          });
  ...
  const reject = (id, comments) => {
    ...
    axios
      .put("http://localhost:5000/admin/denySuppliers", {
        _id: id,
        comments: comments,
        username: window.sessionStorage.getItem("adminName"),
      })
                    ...
                    <div className="flex flex-col gap-5">
                      {supplier.filename !== "" ? (
                        <a
                          className="px-4 py-2 bg-[#FCBD16] rounded-md font-semibold "
                          target="_blank"
                          href={`http://localhost:5000/admin/downloadFiles/${supplier._id}`}
                        >
                          Download
                        </a>
                      ) : (
